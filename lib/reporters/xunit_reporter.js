...
class XUnitReporter {
  ...
  getTestResultNode(document, result) {
    ...
    if (result.file) {
      resultNode.setAttribute('file', result.file);
    }
    resultNode.setAttribute('foobar', result.foobar);
    resultNode.setAttribute('entireTest', result.entireTest);
    resultNode.setAttribute('totalResult', JSON.stringify(result));
